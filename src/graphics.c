...
Uint32 flags =0;
	...
	if(SDL_Init(SDL_INIT_EVERYTHING) !=0){
        slog("Unable to initilazie SDL system: %s", SDL_GetError());
        return;
    }
    atexit(SDL_Quit);
	if (fullscreen)
...
void gt_graphics_render_surface_to_screen(SDL_Surface *surface, SDL_Rect srcRect, int x, int y)
{
	...
	int w,h;
	if(!__gt_graphics_texture){
		slog("gt_graphics_render_surface_to_screen: no texture provided");
		return;
	}
	if(!surface){
		slog("gt_graphics_render)surface_to_screen: no surface provided");
		return;
	}
